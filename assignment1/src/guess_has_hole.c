 ...
 * Works by filtering out options based on smallest maximums and largest minimums for the features.
 * Then uses the range method for any remaining options.
 *
 * input:
...
int hasHoles(double features[NUM_FEATURES]) {
    ...
    // Anything 2 holes must be an 8
    if (holeCount == 2) {
        return 8;
    }

    // These features are dinstinctly identifying enough to allow making a decision here.
    ...
    // If the identified yet, run the range method.
    ...
    // Rather than declare a literal 2D Matrix of values, it seems more organised to create single arrays and assign them individually.
    ...
    */
    ...
    // Filter out options based on Smallest Maxes and Largest Mins
    ...
    // If there were no more options left, print -1.
